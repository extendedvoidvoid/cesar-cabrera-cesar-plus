# CENTRAL POST TRACKER — Master Video Portfolio & Publishing Tracker

> **Sovereign post inventory for César+ / CraftCut.**  
> Structured for **extreme ease of scanning**, **zero-overhead local/online file tracking**, and **literal copy-paste deployment** across LinkedIn, YouTube, Instagram, and Facebook.

---

## 📅 Post Workflow (No-Friction Steps)
1. 📂 **Select:** Locate the local file from the **Local Path** below.
2. 🔗 **Verify:** Spot-check that the file works via the **Live CDN URL**.
3. 📝 **Copy:** Copy the pre-formatted description for your target platform (YouTube / Instagram / Facebook / LinkedIn).
4. 🚀 **Post:** Paste under **Watch** mode. Target publishing window: **5:00 PM – 9:00 PM CET** (11:00 AM EST / 8:00 AM PST) to capture both US coasts and Europe.
5. 📌 **Mark:** Check `[x] Posted` in the inventory table below.

---

## 📊 Post Inventory & Sheet

| ID | Title / Showcase | Language / Mode | Local File Path | Live CDN URL | Target Location Signals | Status |
|----|------------------|-----------------|-----------------|--------------|--------------------------|--------|
| **Vid 01** | **Chanel Absence Essay** | French / Haute Couture | `/Users/alexphoenix/projects/atelier-synesthesie/video1.mp4` | [video1.mp4](https://extendedvoidvoid.github.io/cesar-cabrera-cesar-plus/video1.mp4) | `Paris, FR` / `SoHo, NY` | `[ ] Posted` |
| **Vid 02** | **Saint Laurent Printemps 2026** | French / Fashion Criticism | `/Users/alexphoenix/projects/atelier-synesthesie/video2.mp4` | [video2.mp4](https://extendedvoidvoid.github.io/cesar-cabrera-cesar-plus/video2.mp4) | `Paris, FR` / `Beverly Hills, CA` | `[ ] Posted` |
| **Vid 03** | **Kanye West Graduation 2026** | English / Album Analysis | `/Users/alexphoenix/projects/atelier-synesthesie/video3.mp4` | [video3.mp4](https://extendedvoidvoid.github.io/cesar-cabrera-cesar-plus/video3.mp4) | `SoHo, NY` / `Los Angeles, CA` | `[ ] Posted` |
| **Vid 04** | **Olivia Rodrigo (Third Album)** | English / Music Criticism | `/Users/alexphoenix/projects/atelier-synesthesie/olivia_rodrigo.mp4` | [olivia_rodrigo.mp4](https://extendedvoidvoid.github.io/cesar-cabrera-cesar-plus/olivia_rodrigo.mp4) | `SoHo, NY` / `Los Angeles, CA` | `[ ] Posted` |
| **Vid 05** | **Nirvana (In Utero 1993)** | English / Album Analysis | `/Users/alexphoenix/projects/atelier-synesthesie/nirvana_in_utero.mp4` | [nirvana_in_utero.mp4](https://extendedvoidvoid.github.io/cesar-cabrera-cesar-plus/nirvana_in_utero.mp4) | `SoHo, NY` / `Seattle, WA` | `[ ] Posted` |
| **Vid 06** | **Louis Vuitton (Marc Jacobs & Kate Moss)** | English / Fashion Criticism | `/Users/alexphoenix/projects/atelier-synesthesie/louis_vuitton_kate_moss.mp4` | [louis_vuitton_kate_moss.mp4](https://extendedvoidvoid.github.io/cesar-cabrera-cesar-plus/louis_vuitton_kate_moss.mp4) | `Paris, FR` / `SoHo, NY` | `[ ] Posted` |

---

## 💡 Instagram Algorithm & US Geo-Targeting Rules (Mosseri Law - Aug 2026)

When posting from France to reach **US East Coast (NYC/Miami)** and **West Coast (LA/SF)** audiences:

1. **Hashtags Are Context, Not Growth Hacks**: Use **3 to 5 hyper-specific hashtags max**. Do NOT paste blocks of 20–30 tags (flagged as spam).
2. **Explicit Geotag Signal**: Set location on post/reel to `SoHo, New York` or `Los Angeles, California` to override IP geofencing.
3. **Audio Transcript indexing**: Meta's AI transcribes spoken English audio and indexes reel topic automatically.
4. **Publishing Window**: Post between **17:00 – 21:00 CET** (11:00 AM – 3:00 PM EST / 8:00 AM – 12:00 PM PST).

---

## 📝 Multi-Platform Video Copy & Descriptions

### 📌 Vid 01 — Chanel Absence (Haute Couture)

#### 🔹 YouTube Shorts / Description
```text
César+ · Chanel Absence Essay. Visual narrative exploring the presence of absence in haute couture cinematography.

Location Tag: Paris, France / New York, USA
#Chanel #HauteCouture #CesarPlus #FashionFilm #Cinema
```

#### 🔹 Instagram Reel Copy
```text
César+ · Chanel Absence Essay. Exploring the geometry of silence and haute couture cinema. 

📍 Location: SoHo, New York
#Chanel #HauteCouture #FashionFilm #Cinematography #CesarPlus
```

#### 🔹 Facebook / LinkedIn Copy
```text
Démonstration technique César+ : essai cinématique sur l'esthétique du silence et l'absence dans la Haute Couture Chanel. Moteur de rendu autonome et alignement typographique.

Technical demonstration: cinematic essay on the aesthetics of silence and absence in Chanel Haute Couture. Sovereign rendering engine and vertical typographic layout.
```

---

### 📌 Vid 02 — Saint Laurent Printemps 2026

```text
Saint Laurent Printemps 2026 par Anthony Vaccarello n'est pas un simple défilé : c'est l'étude architecturale d'une silhouette nocturne et d'une élégance souveraine.

Saint Laurent Spring 2026 by Anthony Vaccarello is not a simple fashion show: it is the architectural study of a midnight silhouette and sovereign elegance.

Saint Laurent Primavera 2026 di Anthony Vaccarello non è una semplice sfilata: è lo studio architettonico di una silhouette notturna e di una sovrana eleganza.

Saint Laurent Primavera 2026 de Anthony Vaccarello no es un simple desfile: es el estudio arquitectónico de una silueta nocturna y de una elegancia soberana.

#SaintLaurent #YSL #ParisFashionWeek
```

---

### 📌 Vid 03 — Kanye West Graduation (2026 Essay)

```text
La pochette de Graduation par Takashi Murakami n'est pas une simple illustration : c'est le sceau Superflat marquant l'ascension culturelle de Kanye West.

Kanye West's Graduation sleeve by Takashi Murakami is not a simple illustration: it is the Superflat seal marking West's cultural ascension.

La copertina di Graduation di Takashi Murakami non è un semplice poster : è il sigillo Superflat che sancisce l'ascesa culturale di Kanye West.

La portada de Graduation de Takashi Murakami no es un simple cartel : es el sello Superflat que marca el ascenso cultural de Kanye West.

#KanyeWest #TakashiMurakami #Superflat
```

---

### 📌 Vid 04 — Olivia Rodrigo (Third Album 2026)

```text
La pochette du troisième album d'Olivia Rodrigo par Chad Moore n'est pas une simple photographie : c'est le sceau visuel d'une maturité pop affirmée.

Olivia Rodrigo's third album sleeve by Chad Moore is not a simple photograph: it is the visual seal of a maturing pop authority.

La copertina del terzo album di Olivia Rodrigo firmata Chad Moore non è una semplice fotografia: è il sigillo visivo di una matura autorità pop.

La portada del tercer álbum de Olivia Rodrigo por Chad Moore no es una simple fotografía: es el sello visual de una madura autoridad pop.

#OliviaRodrigo #ChadMoore #AlbumCover
```

---

### 📌 Vid 05 — Nirvana (In Utero 1993)

```text
La pochette d'In Utero par Robert Fisher n'est pas une simple illustration : c'est le sceau viscéral marquant le refus du mythe commercial par Kurt Cobain.

Nirvana's In Utero sleeve by Robert Fisher is not a simple illustration: it is the visceral seal marking Kurt Cobain's refusal of commercial myth.

La copertina di In Utero di Robert Fisher non è un semplice poster : è il sigillo viscerale che sancisce il rifiuto del mito commerciale da parte di Kurt Cobain.

La portada de In Utero de Robert Fisher no es un simple cartel : es el sello visceral que marca el rechazo del mito commercial por parte de Kurt Cobain.

#Nirvana #InUtero #KurtCobain
```

---

### 📌 Vid 06 — Louis Vuitton (Marc Jacobs & Kate Moss)

```text
Le défilé Louis Vuitton par Marc Jacobs n'est pas un simple spectacle : c'est le théâtre d'une sensualité glacée incarnée par Kate Moss.

Marc Jacobs's Louis Vuitton show is not a simple spectacle: it is the theater of a frigid sensuality embodied by Kate Moss.

La sfilata Louis Vuitton di Marc Jacobs non è un semplice spettacolo : è il teatro di una sensualità glaciale incarnata da Kate Moss.

El desfile de Louis Vuitton de Marc Jacobs no es un simple espectáculo: es el teatro de una sensualidad gélida encarnada por Kate Moss.

#LouisVuitton #KateMoss #MarcJacobs
```
